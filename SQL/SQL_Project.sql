...
SELECT
    ...
    CASE
        ...
        WHEN EXTRACT(YEAR FROM CUSTOMER_CREATION_DATE) >= 2011 THEN 'C'
...
WHERE
    OI.ORDER_ID = (
        ...
        -- WHERE OI2.CARTON_ID = 10
        GROUP BY OI2.ORDER_ID
        HAVING SUM(P2.LEN * P2.WIDTH * P2.HEIGHT) <= (
            SELECT LEN * WIDTH * HEIGHT
            FROM CARTON
            WHERE CARTON_ID = 10
        )
        ORDER BY SUM(P2.LEN * P2.WIDTH * P2.HEIGHT) DESC
        LIMIT 1
    )
GROUP BY
    OI.ORDER_ID
...
WHERE
    OH.ORDER_ID % 2 = 0
    AND LEFT(A.PINCODE, 1) NOT LIKE '5%'
GROUP BY
